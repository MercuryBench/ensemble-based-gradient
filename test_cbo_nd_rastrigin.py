...
Phi = lambda z: 0.5*np.linalg.norm(z-1, axis=0)**4# just for safety
DPhi = lambda z: z

# Rastrigin
A = 3
Phi = lambda z: A*d + np.sum(z**2 - A*np.cos(2*pi*z), axis=0)
...
params = {}
params["alpha"] = 1000.0 # weight exponent of cost function exp(-alpha*Phi)
...
params["sig"] = 1.0 # coefficient of noise
params["kappa"] = 0*0.5 # coefficient of gradient drift term
params["avar"] = 1000000.0

N_ens = 100 # size of ensemble
J = N_ens
...
# initial ensemble
us0_global = np.random.uniform(-12,-2,(d,J)) # hard for functions with minimum at (0,0)
#us0_global = np.array([[-4,-1.5],[-4,-2.5]])
#us0_global = np.random.uniform(-3,+3,(d,J)) # hard for Himmelblau
...
for i in range(N-1):
    ...
    us_list[:,:,i+1] = step_gradCBO(us_list[:, :, i], Phi, params,tau=tau, maxstep="var", noise="component", memory=memory)
    memory = us_list[:,:,i]
...
plt.figure()
plt.subplot(2,1,1)
plt.title("1st component of particles")
plt.plot(us_list[0,:,:].T)
plt.subplot(2,1,2)
plt.title("2nd component of particles")
...
print(f"weighted mean: norm(wmean)={np.linalg.norm(w_mean[:,-1])}")
print(f"Phi(wmean)={Phi(w_mean[:,-1])}")
