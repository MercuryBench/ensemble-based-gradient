...
if __name__ == "__main__":
    ...
    avar=1000.0
    scaledversion = True
    ret = inferGradientAndHess(xs, vs, hessian=hessian,ind=0,penalizedistance=penalizedistance, additionalvariance=avar, scaledversion=scaledversion)
    ...
    for k in range(J):
        ret = inferGradientAndHess(xs, vs, hessian=hessian,ind=k,penalizedistance=penalizedistance, retOnlyMatrix=True, additionalvariance=avar, scaledversion=scaledversion)
        mat, vec = ret['A'], ret['b']
    ...
    for k in range(J):
        ret = inferGradientAndHess(xs, vs, hessian=hessian,ind=k,penalizedistance=penalizedistance, additionalvariance=avar, scaledversion=scaledversion)
    ...
    if dim == 2:
        ...
        wmean = xs[:,4]
        ret_mean = inferGradientAndHess(xs, vs, hessian=hessian,ind=4,penalizedistance=penalizedistance, additionalvariance=avar, scaledversion=scaledversion)
